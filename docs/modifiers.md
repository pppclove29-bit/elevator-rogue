# Modifiers & Relics — 설계 카탈로그 (구현 완료)

> ✅ Step 7 메타 시스템 구현 완료. 이 문서는 초기 설계 안과 결정사항 아카이브.
> 현재 코드 진실원천: `src/meta/modifiers.ts`, `src/meta/relics.ts`, `src/meta/events.ts` (공휴일 포함).
> 게임 내 실제 풀: `/docs.html` 대시보드 참고.
>
> 이후 추가된 사항 (이 문서엔 미반영):
> - 운송 수단 렐릭 5장 (에스컬레이터/지하철/헬리포트)
> - 인력/시설 렐릭 7장 (경비/청소부/화장실)
> - 보스 day 이벤트 5장 (Day 7/14/21/28/35+)
> - 공휴일 이벤트 12장 (1/1 신정 ~ 12/31 송년)
> - 빌딩 테마 4종 (오피스/호텔/병원/공항) — 테마 순차 해금
